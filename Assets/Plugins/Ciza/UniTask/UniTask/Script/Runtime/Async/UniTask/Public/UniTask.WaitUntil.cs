...
namespace CizaUniTask
{
	public readonly partial struct UniTask
	{
		...
		private class WaitUntilPromise : PlayerLoopReusablePromiseBase
		{
			...
			public WaitUntilPromise(Func<bool> predicate, PlayerLoopTiming timing, CancellationToken cancellationToken) : base(timing, cancellationToken)
			{
		...
		private class WaitWhilePromise : PlayerLoopReusablePromiseBase
		{
			...
			public WaitWhilePromise(Func<bool> predicate, PlayerLoopTiming timing, CancellationToken cancellationToken) : base(timing, cancellationToken)
			{
				this.predicate = predicate;
			}
		...
		private class WaitUntilValueChangedUnityObjectPromise<T, U> : PlayerLoopReusablePromiseBase<U>
		{
			...
			public WaitUntilValueChangedUnityObjectPromise(T target, Func<T, U> monitorFunction, IEqualityComparer<U> equalityComparer, PlayerLoopTiming timing, CancellationToken cancellationToken) : base(timing, cancellationToken)
			{
				this.target = target;
		...
		private class WaitUntilValueChangedStandardObjectPromise<T, U> : PlayerLoopReusablePromiseBase<U> where T : class
		{
			...
			public WaitUntilValueChangedStandardObjectPromise(T target, Func<T, U> monitorFunction, IEqualityComparer<U> equalityComparer, PlayerLoopTiming timing, CancellationToken cancellationToken) : base(timing, cancellationToken)
			{
